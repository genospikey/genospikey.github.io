...
AudioInfo.prototype.update = function (dt){
    ...
    for(var x = 0; x<this.numSlices; x++){
        var index = Math.floor(this.dataArrayFreq.length/(this.numSlices*this.numSlices) * x*x);
        // index = Math.floor(Math.pow(x/this.numSlices,3.21928)*this.dataArrayFreq.length);
        var val = this.dataArrayFreq[index];//1.12*Math.pow(10,-8)*Math.pow(this.dataArrayFreq[index],3.2);
        ...
        if(val > this.max[x]){
            ...
            this.max[x] = this.max[x] * (1 - dt/500);
    }
...
AudioInfo.prototype.addUpdateEvent = function(f){
    ...
    return this.updateEvents[this.updateEvents.length-1];
};
...
AudioInfo.prototype.changeSource = function(a,t){
    if(t == 'mic'){
        console.log(a);
        this.source = this.audioCtx.createMediaStreamSource(a);
        this.gain = this.audioCtx.createGain();
        this.gain.gain.setValueAtTime(10,this.audioCtx.currentTime);
